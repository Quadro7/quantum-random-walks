...
if __name__ == "__main__":
    ...
    ypoints = []
    for i in range(181):
        distribution = quantum_walk(T=steps, phi=90, beta=math.pi / 180 *i)
        xpoints.append(i)
        ypoints.append(distribution[2])
    # PLOTTING
    plt.xlabel('Beta (in degrees)')
    plt.ylabel('Variance')
    #plt.xticks(np.arange(-steps, steps+1, step=1))
